...
unique(DT.import$Страны.поставщики)
# цвет по месяцам для легенды графика
countries <- as.factor(unique(DT.import$Страны.поставщики))
plot(x = x, y = y,
     pch = 21, bg = cls[as.factor(DT.import$Страны.поставщики)],
     axes = F, ylim = c(0, 200000), xlim = c(0, 700000),
     xlab = 'Стоимость поставки', 
     ylab = 'Масса поставки')
# горизонтальная ось 
axis(side = 1, 
     pos = 0, 
     at = seq(0, 650000, by = 50000),
     labels = seq(0, 650000, by = 50000))
# вертикальная ось
axis(side = 2, 
     pos = 0, 
     at = seq(0, 180000, by = 20000),
     labels = c('', seq(20000, 180000, by = 20000)),
     las = 2)
# стрелки на концах осей
arrows(x0 = c(0, 650000), y0 = c(180000, 0),
       x1 = c(0, 700000), y1 = c(200000, 0))
# легенда
legend('topright', legend = countries, fill = cls[countries])
# заголовок
mtext(text = 'Разброс значений по странам', 
      side = 3, line = 2, font = 2)
mtext(text = '(в легенде указаны страны)', 
...
```{r}
xyplot(x ~ y, data = DT.import, auto.key = T,
       groups = Страны.поставщики,
       ylab = 'Масса поставки',
       xlab = 'Стоимость поставки') 
xyplot(x ~ y | Страны.поставщики, data = DT.import,
       ylab = 'Масса поставки',
       xlab = 'Стоимость поставки') 
```
